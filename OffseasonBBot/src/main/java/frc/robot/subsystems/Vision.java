...
//Returns distance of AprilTag from robot if the robot has a target and it is not very ambiguous
...
//If the tags are farther away, standard deviations will be higher
...
//Returns angle from the tag about the z axis of the robot
...
//Returns distance from a specific AprilTag
...
//returns estimated pose of the robot using AprilTags
...
//returns the best AprilTag to target
  public PhotonTrackedTarget getBestTarget() {
    var result = m_camera.getLatestResult();
    if (!result.hasTargets()) return null;
    return result.getBestTarget();
  }
//Returns if the camera sees any AprilTags
  public boolean getHasTarget() {
    var result = m_camera.getLatestResult();
    return result.hasTargets();
  }
//Returns specific target if the camera sees it.
