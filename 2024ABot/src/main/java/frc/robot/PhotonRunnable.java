...
public class PhotonRunnable implements Runnable {
  ...
  public PhotonRunnable(String[] cameraNames, Transform3d[] robotToCameras, BiConsumer<Pose2d, Double> poseConsumer,
    ...
    // for (int i = 0; i <=cameraNames.length-1; i++) {
    ...
    for (int i = 0; i < cameraNames.length; i++) {
      var cameraTable = NetworkTableInstance.getDefault().getTable("photonvision1").getSubTable(cameraNames[i]);
      ...
      photonPoseEstimators[i] = new PhotonPoseEstimator(
          layout, PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR, new PhotonCamera(cameraNames[i]), robotToCameras[i]);
    }
  }
        ...
        if (photonResults.hasTargets() && (photonResults.targets.size() > 1
          ...
          photonPoseEstimator.update(photonResults).ifPresent(estimatedRobotPose -> {
            ...
            if (estimatedPose.getX() > 0.0 && estimatedPose.getX() <= 16.54175
                && estimatedPose.getY() > 0.0 && estimatedPose.getY() <= 8.013) {
